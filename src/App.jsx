...
import { fetchUserFromDB, fetchCardsForUser, fetchTransactionsForCards, createTransactionInDB, updateCardBalanceInDB, createInvoiceInDB, fetchInvoicesForUser, updateInvoiceStatusInDB, findTargetCardOrUser } from './utils/simulation';
...
export default function App() {
  ...
  const handleTransferSubmit = async (e) => {
    ...
    try {
      // Find receiver's card across ALL devices (Supabase-first)
      const targetResult = await findTargetCardOrUser(transferTarget);
      let receiverCard = null;

      if (targetResult) {
        if (targetResult.type === 'card') {
          receiverCard = targetResult.data;
        } else if (targetResult.type === 'user') {
          receiverCard = targetResult.card;
        }
      }
  ...
  const handleInvoiceSubmit = async (e) => {
    ...
    try {
      // Find receiver's user across ALL devices (Supabase-first)
      const targetResult = await findTargetCardOrUser(invoiceTarget);
      let receiverUser = null;

      if (targetResult) {
        if (targetResult.type === 'user') {
          receiverUser = targetResult.user;
        } else if (targetResult.type === 'card') {
          // If searched by card, fetch the user who owns that card
          receiverUser = await fetchUserFromDB(targetResult.data.passport_code);
        }
      }
  ...
  const handleAcceptInvoice = async (inv) => {
    ...
    try {
      // Find sender's card across ALL devices (Supabase-first)
      const targetResult = await findTargetCardOrUser(inv.sender_passport);
      let senderCard = null;

      if (targetResult) {
        if (targetResult.type === 'user') {
          senderCard = targetResult.card;
        } else if (targetResult.type === 'card') {
          senderCard = targetResult.data;
        }
      }
