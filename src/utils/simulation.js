...
// NEW FLAWLESS FUNCTION: Find any card or user across ALL devices (Supabase first, then local fallback)
export async function findTargetCardOrUser(targetValue) {
  const cleanTarget = targetValue.trim().replace(/\s+/g, '');
  
  try {
    // 1. Try finding by card number in Supabase
    const { data: cardsData, error: cardErr } = await supabase
      .from('bank_cards')
      .select('*');
    
    if (!cardErr && cardsData) {
      const card = cardsData.find(c => c.card_number.replace(/\s+/g, '') === cleanTarget);
      if (card) return { type: 'card', data: card };
    }

    // 2. Try finding user by passport code in Supabase
    const { data: userData, error: userErr } = await supabase
      .from('bank_users')
      .select('*');
    
    if (!userErr && userData) {
      const user = userData.find(u => u.passport_code === targetValue.trim());
      if (user) {
        const userCard = cardsData?.find(c => c.passport_code === user.passport_code && c.card_type === 'personal') || cardsData?.find(c => c.passport_code === user.passport_code);
        return { type: 'user', user, card: userCard };
      }
    }
  } catch (e) {
    console.error("Cloud search error, falling back to LocalStorage:", e);
  }

  // Local Fallback
  const db = getLocalDB();
  const card = db.cards.find(c => c.card_number.replace(/\s+/g, '') === cleanTarget);
  if (card) return { type: 'card', data: card };

  const user = db.users.find(u => u.passport_code === targetValue.trim());
  if (user) {
    const userCard = db.cards.find(c => c.passport_code === user.passport_code && c.card_type === 'personal') || db.cards.find(c => c.passport_code === user.passport_code);
    return { type: 'user', user, card: userCard };
  }
